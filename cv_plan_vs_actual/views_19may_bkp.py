...
def table_view(request):
	...
	if serialdata.is_valid():
		...
		data=obj.table_view(plant_id,shop_type,shop,Status,shift,from_date,to_date)
		data1=obj.distinct_plant()
		for j in data:
			for i in data1:
				if str(j['plant'])==str(i['plant_id']):
					j['plant']=i['plant_name']
		data2=obj.distinct_shop(plant_id,shop_type)
		for j in data:
			for i in data2:
...
def table_view_export(request):
                ...
                excel_data = obj.table_view_export(plant_id,shop_type,shop,Status,shift,from_date,to_date)
                data1=obj.distinct_plant()
                for j in excel_data:
                    for i in data1:
                        if str(j['plant'])==str(i['plant_id']):
...
def plan_vs_actual_daily_count(request):
	...
	if serialdata.is_valid():
		...
		from_date=data['from_date']
		to_date=data['to_date']
		data=obj.plan_vs_actual_daily_count(plant_id,shop_type,shop,Status,shift,from_date,to_date)
		return Response(data,status=status.HTTP_200_OK)
	else:
		return Response("error",status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def plan_vs_actual_hr_month_yr_count(request):
	...
	if serialdata.is_valid():
		...
		from_date=data['from_date']
		to_date=data['to_date']
		data=obj.plan_vs_actual_hr_month_yr_count(plant_id,shop_type,shop,Status,shift,duration,req_duration,from_date,to_date)
		return Response(data,status=status.HTTP_200_OK)
	else:
		return Response("error",status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def table_view(request):
	...
	if serialdata.is_valid():
		...
		size=data['size']
		start=(start-1)*(size)
		data=obj.table_view(plant_id,shop_type,shop,Status,shift,from_date,to_date,searchtext,start,colorder,coltype,size,shop_type_small)
		plant_dict={}
		for i in data1:
