...
#TOP Filters 1: plant
    def distinct_plant(self):
        ...
        res_vin=es.search(index="flat_mfg_digital_twin_api",body=query,size=10000,request_timeout=120)
        list=[]
...
#TOP Filters 4: Status
    def distinct_status(self,plant_id,shop_type):
        ...
        es = Elasticsearch(["http://elastic:elastic@172.31.158.159:9200/"])	
        print(query)
        res_vin=es.search(index="flat_mfg_plan_vs_actual_shoptype_status",body=query,size=10000,request_timeout=120)
        list=[]
        x2=res_vin['aggregations'].get('groupby').get('buckets')
        for i in x2:
            data={}
            ...
            list.append(data)
        custom_order = ['PLANNED', 'DROPPED', 'BOOKED', 'WTCL', 'SFLT', 'ROLLED']
...
#Graph 1:
    def plan_vs_actual_daily_count(self,plant_id,shop_type,shop,Status,shift,line,from_date,to_date):
        ...
        name=index_name(self,from_date,to_date)
        res_vin1=es.search(index=name,body=plan_query,request_timeout=120)
        res_vin2=es.search(index=name,body=actual_query,request_timeout=120)
...
#custome table_view
    def table_view(self,plant_id,shop_type,shop,Status,shift,line,from_date,to_date,searchtext,start,colorder,coltype,size,shop_type_small):
        ...
        if searchtext=='undefined' :
            print("searchtext undefined & fdate & tdate == or not")
            count_query={
		"size" : 0,
		"query" : {
			"bool" : {
